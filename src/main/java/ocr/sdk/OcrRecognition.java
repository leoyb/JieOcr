...
public class OcrRecognition {
    ...
    public List<DetectedObjects.DetectedObject> recognizeImg(Image image) throws Exception {
        ...
        ZooModel detectionModel = ModelZoo.loadModel(Criteria.builder().optEngine("PaddlePaddle").optModelName("inference").setTypes(Image.class, DetectedObjects.class).optModelPath(Paths.get("src/models/ch_PP-OCRv3_det_infer")).optTranslator(new PpWordDetectionTranslator()).optProgress(new ProgressBar()).build());
        ZooModel  recognitionModel = ModelZoo.loadModel(Criteria.builder().optEngine("PaddlePaddle").optModelName("inference").setTypes(Image.class, String.class).optModelPath(Paths.get("src/models/ch_PP-OCRv3_rec_infer")).optProgress(new ProgressBar()).optTranslator(new PpWordRecognitionTranslator()).build());
